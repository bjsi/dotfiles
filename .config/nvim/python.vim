...
augroup python_commands
    autocmd!
    autocmd FileType python set shortmess+=c
    execute 'autocmd FileType python xmap <silent> <buffer> ' . dic["code_actions"] . ' <Plug>(coc-codeaction-selected)'
    ...
    execute 'autocmd FileType python nmap <silent> <buffer> ' . dic["go_to_def"] . ' <Plug>(coc-definition)'
    execute 'autocmd FileType python nmap <silent> <buffer> ' . dic["rename"] . ' <Plug>(coc-rename)'
    " execute 'autocmd FileType python nmap <silent> <buffer> ' . dic["format"] . ' <Plug>(omnisharp_code_format)'
    execute 'autocmd FileType python nmap <silent> <buffer> ' . dic["find_implementations"] . ' <Plug>(coc-implementation)'
    ...
    autocmd FileType py let g:strip_whitespace_on_save = 1
    autocmd FileType py let g:strip_whitespace_confirm = 0
augroup END
